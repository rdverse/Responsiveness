...
def save_masks(frame_no, masks, boxes, track_ids, confs, clss, frame, folder_path):
    for mask, box, track_id, conf,cls in zip(masks, boxes, track_ids, confs,clss):
            ...
            folder_path_mask = os.path.join(folder_path, str(track_id))
            if not os.path.exists(folder_path_mask):
                os.makedirs(folder_path_mask)
            cv2.imwrite(os.path.join(folder_path_mask, f"{frame_no}.png"), frame_masked)
...
if __name__ == "__main__":
    ...
    # Open the video file
    video_path = "/sam_box/inputs_huma/"
    output_path = "/sam_box/outputs_huma/masks_frames/"
    ...
    for file_name in tqdm.tqdm(file_names):
        ...
        output_path = "/sam_box/outputs_huma/masks_frames/"
        file_names = os.listdir(video_path)
        file_names = [file for file in file_names if file.endswith(".MTS")]
        #video
        ...
        result = cv2.VideoWriter(f'/sam_box/outputs_huma/{file_name.strip(".MTS")}.avi',
        ...
        while cap.isOpened():
            ...
            if success:
                ...
                if results[0].boxes.id is not None:
                    ...
                    save_masks(frame_count, masks.cpu().numpy(), boxes, track_ids, confs, clss, frame_stash, os.path.join(output_path, file_name.strip(".MTS")))
