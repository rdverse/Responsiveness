...
# torch originally 1.10.2 now downgrading to 1.9.1
import matplotlib
...
results_copy = []
PATH_TO_LABELS = './models/research/object_detection/data/person.pbtxt'
...
print('loading model...')
center_model = hub.load(ALL_MODELS[model_handle])
print('model loaded!')
'''
...
depthModel = torch.hub.load("intel-isl/MiDaS", model_type)
device = torch.device(
    "cuda") if torch.cuda.is_available() else torch.device("cpu")
...
if model_type == "DPT_Large" or model_type == "DPT_Hybrid":
    depthTransform = midas_transforms.dpt_transform
else:
    depthTransform = midas_transforms.small_transform
...
while (cap.isOpened()):
    ...
    if ret:
        if cnt % frame_rate_divider == 0:
            ...
            input_frame_depth = depthTransform(image_np.squeeze()).to(device)

            with torch.no_grad():
                frame_depth_device = depthModel(input_frame_depth)

                frame_depth_device = torch.nn.functional.interpolate(
                    frame_depth_device.unsqueeze(1),
                    size=image_np.squeeze().shape[:2],
                    mode="bicubic",
                    align_corners=False,
                ).squeeze()

            frame_depth = frame_depth_device.cpu().numpy() * 255 / 32
            
            depthHeatmap = np.dstack((frame_depth, frame_depth, frame_depth))
            #cv2.cvtColor(frame_depth, cv2.COLOR_GRAY2BGR)
            ...
            results["keypoints"] = [[[
                ...
                results["detection_keypoint_scores"][i][j]
            ] for j,keyXY in enumerate(keyPerson)] for i,keyPerson in enumerate(results["keypoints"])]

            
            results_copy = results.copy()
            keepIndices = [
                i for i, score in enumerate(results['scores']) if score > 0.3
            ]
            ...
            keepIndices = [
                i for i, val in enumerate(results['detection_classes'])
                if val == 1.0
            ]
            ...
            if not args.initialize:
                if cnt == 0:
                    ...
                    print("Going to the detector")
                    detector.classifyDetections(results, cnt)

            if args.video_output:
                output.write(np.squeeze(frame_keypoints).astype(np.uint8))
                outputDepth.write(np.squeeze(depthHeatmap).astype(np.uint8))
                
                # plt.imsave("cnt {}.png".format(cnt), frame_depth.astype(np.uint8))
            print(cnt)
