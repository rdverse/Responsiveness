...
sLicensed under the MIT License (see LICENSE for details)
...
def display_image_keypoint_mask(image,
                                boxes,
                                keypoints,
                                keypoint_weight,
                                class_ids,
    ...
    if (iskeypointlabel):  # convert the label of joint into coordinate
        ...
        keypoints[:, :,
                  0] = keypoints[:, :,
                                 0] * heatmap_scale_w + x1 * image.shape[1]
        keypoints[:, :,
                  1] = keypoints[:, :,
                                 1] * heatmap_scale_h + y1 * image.shape[0]
