...
def minimize_keypoint_mask(bbox, keypointmask, mini_shape):
    """Resize keypoint_mask to a smaller version to cut memory load.
        ...
        See inspect_data.ipynb notebook for more details.
        """
    mini_mask = np.zeros(mini_shape + (
        keypointmask.shape[2],
        keypointmask.shape[3],
    ),
                         dtype=bool)
...
def expand_mask(bbox, mini_mask, image_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        h = y2 - y1
        w = x2 - x1
        m = resize(m.astype(float), (h, w), interp='bilinear')
        # _positon = np.argmax(m)  # get the index of max in the a
...
def unmold_mask(mask, bbox, image_shape):
    ...
    threshold = 0.5
    y1, x1, y2, x2 = bbox
    mask = resize(mask, (y2 - y1, x2 - x1), interp='bilinear').astype(
        np.float32) / 255.0
...
def unmold_keypoint_mask(keypoints_prob,
                         bbox,
                         image_shape,
                         mask,
